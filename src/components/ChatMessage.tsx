    ...
    if (part.match(/^https?:\/\//)) {
      return (
        <a key={idx} href={part} target="_blank" rel="noopener noreferrer"
          className={`underline underline-offset-2 break-all ${isBot ? "text-primary hover:opacity-80" : "hover:opacity-80"}`}>
          {part}
        </a>
      );
    }
...
const renderMarkdown = (text: string, isBot: boolean): React.ReactNode => {
    ...
    nodes.push(
      <ul key={key} className="list-disc list-inside space-y-0.5 mt-1 mb-1">
        {bulletBuffer.map((b, i) => (
          <li key={i} className="leading-snug">{renderInline(b, isBot)}</li>
        ))}
      </ul>
    );
    bulletBuffer = [];
  ...
  lines.forEach((raw, i) => {
    ...
    // Skip [Section: ...] metadata lines entirely
    ...
    if (headingMatch) {
      ...
      const className = level === 1 ? "font-bold text-base mt-2 mb-0.5"
        : level === 2 ? "font-semibold text-sm mt-1.5 mb-0.5"
        : "font-semibold text-sm mt-1";
      nodes.push(<p key={i} className={className}>{renderInline(headingMatch[2], isBot)}</p>);
      return;
    }
    ...
    // Normal text
    flushBullets(`bl-${i}`);
    nodes.push(<p key={i} className="leading-snug">{renderInline(line, isBot)}</p>);
  });
...
const ChatMessage = ({ message }: ChatMessageProps) => {
  ...
  return (
    <div className={`flex gap-2.5 animate-chat-fade-in ${isBot ? "justify-start" : "justify-end"}`}>
      {isBot && (
